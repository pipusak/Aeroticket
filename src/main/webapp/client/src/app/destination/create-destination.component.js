"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
/**
 * Created by Ruslan on 09.12.2016.
 */
var core_1 = require("@angular/core");
var request_status_1 = require("../shared/request-status");
var CreateDestinationComponent = (function () {
    function CreateDestinationComponent(service) {
        this.service = service;
    }
    CreateDestinationComponent.prototype.createDestination = function (name, lat, lon) {
        var _this = this;
        this.requestStatus = request_status_1.RequestStatus.PENDING;
        this.service.createDestination(name, lat, lon).then(function (dest) {
            _this.requestStatus = request_status_1.RequestStatus.OK;
        }).catch(function (err) {
            _this.requestStatus = request_status_1.RequestStatus.ERROR;
            _this.requestError = err;
            console.log(err);
        });
    };
    CreateDestinationComponent = __decorate([
        core_1.Component({
            selector: 'create-destination',
            templateUrl: 'create-destination.component.html',
            styleUrls: ['../shared/request-status-notification.component.css']
        })
    ], CreateDestinationComponent);
    return CreateDestinationComponent;
}());
exports.CreateDestinationComponent = CreateDestinationComponent;
